...
namespace ProcessSchedulingSimulation{
	NonPreemptivePrioritySchedulingForm::NonPreemptivePrioritySchedulingForm(int noOfProcesses, DynamicSafeArray<Process>& processArray)
	{
		InitializeComponent();
	...
	void NonPreemptivePrioritySchedulingForm::updateReadyQueue(){
	...
	void NonPreemptivePrioritySchedulingForm::simulateExecution() {
		int simulationClockTime = 0;
		Process * thisProc = new Process();
		double lastCompletionTime, firstArrivalTime;
		int i = 0;
		PriorityQueue < Process > * outputQueue = new PriorityQueue < Process > ();
	...
	NonPreemptivePrioritySchedulingForm::~NonPreemptivePrioritySchedulingForm()
	{
		if (readyQueue != 0){
			delete readyQueue;
			readyQueue = 0;
